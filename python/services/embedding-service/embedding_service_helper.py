...
class CLIPEmbedder:
    ...
    def _parse_clip_model_spec(model_spec: str) -> Tuple[str, str]:
        ...
        known_mappings = {
            ...
            "laion/clip-vit-b-16-plus-240-laion2b-s34b-b88k": (
                "ViT-B-16-plus-240",
                "laion2b_s34b_b88k",
            ),
        }
        ...
        for arch, tag in available_models:
            ...
            combined_score = difflib.SequenceMatcher(
                None, norm_spec, norm(f"{arch}-{tag}")
            ).ratio()
        ...
        available_str = ", ".join(
            [f"{arch} (pretrained: {tag})" for arch, tag in available_sample]
        )
